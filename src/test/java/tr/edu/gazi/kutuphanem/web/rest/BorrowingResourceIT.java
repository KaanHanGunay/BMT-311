...
class BorrowingResourceIT {
    ...
    void createBorrowing() throws Exception {
        ...
        restBorrowingMockMvc
            .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(borrowingDTO)))
    ...
    void putNewBorrowing() throws Exception {
        ...
        BorrowingDTO borrowingDTO = borrowingMapper.toDto(updatedBorrowing);

        restBorrowingMockMvc
            .perform(
                put(ENTITY_API_URL_ID, borrowingDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(borrowingDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamBorrowing() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restBorrowingMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(borrowingDTO)))
    ...
    void patchNonExistingBorrowing() throws Exception {
        ...
        restBorrowingMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, borrowingDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(borrowingDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamBorrowing() throws Exception {
        ...
        restBorrowingMockMvc
            .perform(
                patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(TestUtil.convertObjectToJsonBytes(borrowingDTO))
            )
            .andExpect(status().isMethodNotAllowed());
